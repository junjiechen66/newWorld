...
**更新** 新增完整的部署自动化系统，支持一键部署、快速部署、镜像构建、服务重启等多种部署模式，以及完善的部署验证和监控功能。现代化的 Nginx 反向代理配置提供了完整的静态资源托管、API 代理、Gzip 压缩和缓存控制功能。
...
END2["部署验证<br/>自动部署验证"]
END3["服务检查<br/>check.js"]
END4["SSH 连接<br/>ssh.js"]
END5["文件传输<br/>sftp.js"]
end
APP --> CFG
APP --> JWT
APP --> CORS
...
- **新增** 容器编排架构：明确区分内部应用服务（仅内部访问）和外部 Nginx 代理服务（对外暴露端口）。
...
- **新增** 容器编排架构：应用服务仅内部暴露（8080端口），Nginx 服务对外暴露（80端口），实现清晰的网络隔离。
...
Run --> Expose["应用: 8080 (内部)<br/>Nginx: 80 (外部)"]
...
- **API 代理**：将 `/api/` 前缀的请求代理到后端应用容器，支持超时配置和头部传递。
- **Gzip 压缩**：启用静态资源压缩，支持多种 MIME 类型，提升传输效率。
- **缓存控制**：为静态资源设置长期缓存策略（30天），减少带宽消耗。
- **接口文档代理**：支持 Knife4j 在线文档的访问，包括 doc.html 和 v3 接口。
- **认证接口处理**：特殊处理 /auth/ 前缀的认证接口，确保登录注册功能正常工作。
...
## 结论
通过 Docker Compose 可快速完成"新世界"项目的生产部署，配合 Nginx 反向代理与 SSL 证书，可满足中小规模生产的可用性与安全性需求。新增的部署自动化系统进一步提升了部署效率和可靠性，支持多种部署模式和完善的监控验证功能。现代化的容器编排架构实现了应用服务与代理服务的清晰分离，提高了系统的安全性和可维护性。建议在生产环境中完善监控与日志体系、实施严格的变更与回滚流程，并根据业务增长逐步演进到 Kubernetes 等更高阶的编排平台。

**更新** 新增的部署自动化系统显著提升了部署的自动化程度和运维效率，现代化的 Nginx 配置和容器编排架构提供了更好的性能和安全性。
