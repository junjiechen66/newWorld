# 认证接口

<cite>
**本文引用的文件**
- [AuthController.java](file://backend/src/main/java/com/newworld/controller/AuthController.java)
- [AuthService.java](file://backend/src/main/java/com/newworld/service/AuthService.java)
- [AuthServiceImpl.java](file://backend/src/main/java/com/newworld/service/impl/AuthServiceImpl.java)
- [LoginRequest.java](file://backend/src/main/java/com/newworld/dto/LoginRequest.java)
- [UpdateUserRequest.java](file://backend/src/main/java/com/newworld/dto/UpdateUserRequest.java)
- [ChangePasswordRequest.java](file://backend/src/main/java/com/newworld/dto/ChangePasswordRequest.java)
- [JwtUtil.java](file://backend/src/main/java/com/newworld/common/JwtUtil.java)
- [AuthInterceptor.java](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java)
- [WebMvcConfig.java](file://backend/src/main/java/com/newworld/config/WebMvcConfig.java)
- [Result.java](file://backend/src/main/java/com/newworld/common/Result.java)
- [GlobalExceptionHandler.java](file://backend/src/main/java/com/newworld/common/exception/GlobalExceptionHandler.java)
- [application.yml](file://backend/src/main/resources/application.yml)
- [User.java](file://backend/src/main/java/com/newworld/entity/User.java)
- [UserMapper.java](file://backend/src/main/java/com/newworld/mapper/UserMapper.java)
- [auth.js](file://frontend/src/api/auth.js)
- [auth.store.js](file://frontend/src/stores/auth.js)
- [request.js](file://frontend/src/utils/request.js)
</cite>

## 目录
1. [简介](#简介)
2. [项目结构](#项目结构)
3. [核心组件](#核心组件)
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [依赖分析](#依赖分析)
7. [性能考量](#性能考量)
8. [故障排查指南](#故障排查指南)
9. [结论](#结论)
10. [附录](#附录)

## 简介
本文件为认证接口的详细API文档，覆盖用户登录、注册、退出、当前用户信息查询与修改、密码修改等完整认证能力。重点说明：
- 接口定义与请求/响应格式
- JWT Token 的生成、验证与失效处理
- 认证拦截器工作原理与请求头传递方式
- 用户信息修改与密码修改的业务流程与安全验证
- 常见错误处理与安全建议

## 项目结构
后端采用 Spring Boot + MyBatis-Plus 架构，认证相关模块分布如下：
- 控制层：AuthController 提供 /api/auth/* 接口，包含登录、注册、用户信息查询、用户信息修改、密码修改、退出等接口
- 服务层：AuthService 及其实现类负责业务逻辑，新增用户信息修改与密码修改方法
- 基础设施：JwtUtil 提供 JWT 编解码；AuthInterceptor 实现全局认证拦截
- 配置：WebMvcConfig 注册拦截器与跨域策略
- 统一响应：Result 封装统一返回结构
- 异常处理：GlobalExceptionHandler 统一捕获业务异常
- DTO 层：新增 UpdateUserRequest 和 ChangePasswordRequest 用于参数验证

```mermaid
graph TB
subgraph "前端"
FE_API["前端 API 模块<br/>auth.js"]
FE_STORE["Pinia Store<br/>auth.store.js"]
FE_REQ["Axios 请求封装<br/>request.js"]
end
subgraph "后端"
AC["控制器<br/>AuthController"]
ASI["服务实现<br/>AuthServiceImpl"]
JWT["JWT 工具<br/>JwtUtil"]
INT["拦截器<br/>AuthInterceptor"]
CFG["MVC 配置<br/>WebMvcConfig"]
RES["统一响应<br/>Result"]
EX["全局异常<br/>GlobalExceptionHandler"]
UM["用户映射<br/>UserMapper"]
ENT["用户实体<br/>User"]
D1["更新用户信息DTO<br/>UpdateUserRequest"]
D2["修改密码DTO<br/>ChangePasswordRequest"]
end
FE_API --> FE_REQ
FE_REQ --> AC
AC --> ASI
ASI --> UM
ASI --> JWT
INT --> JWT
CFG --> INT
ASI --> RES
INT --> RES
EX --> RES
UM --> ENT
AC --> D1
AC --> D2
```

图表来源
- [AuthController.java:1-73](file://backend/src/main/java/com/newworld/controller/AuthController.java#L1-L73)
- [AuthServiceImpl.java:1-97](file://backend/src/main/java/com/newworld/service/impl/AuthServiceImpl.java#L1-L97)
- [JwtUtil.java:1-78](file://backend/src/main/java/com/newworld/common/JwtUtil.java#L1-L78)
- [AuthInterceptor.java:1-78](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java#L1-L78)
- [WebMvcConfig.java:1-53](file://backend/src/main/java/com/newworld/config/WebMvcConfig.java#L1-L53)
- [Result.java:1-90](file://backend/src/main/java/com/newworld/common/Result.java#L1-L90)
- [GlobalExceptionHandler.java:1-35](file://backend/src/main/java/com/newworld/common/exception/GlobalExceptionHandler.java#L1-L35)
- [UserMapper.java:1-10](file://backend/src/main/java/com/newworld/mapper/UserMapper.java#L1-L10)
- [User.java:1-95](file://backend/src/main/java/com/newworld/entity/User.java#L1-L95)
- [UpdateUserRequest.java:1-19](file://backend/src/main/java/com/newworld/dto/UpdateUserRequest.java#L1-L19)
- [ChangePasswordRequest.java:1-36](file://backend/src/main/java/com/newworld/dto/ChangePasswordRequest.java#L1-L36)
- [auth.js:1-14](file://frontend/src/api/auth.js#L1-L14)
- [auth.store.js:1-41](file://frontend/src/stores/auth.store.js#L1-L41)
- [request.js:1-56](file://frontend/src/utils/request.js#L1-L56)

## 核心组件
- 认证控制器：提供登录、注册、获取当前用户信息、修改用户信息、修改密码、退出接口
- 认证服务：实现注册、登录校验、用户信息查询、用户信息修改、密码修改
- JWT 工具：生成、解析与校验 Token
- 认证拦截器：拦截请求，校验 Token 并注入当前用户上下文
- MVC 配置：注册拦截器与跨域策略
- 统一响应与异常处理：规范返回结构与错误码
- 参数验证：UpdateUserRequest 和 ChangePasswordRequest 提供字段验证

## 架构总览
认证流程概览：
- 前端通过 Axios 发起请求，自动在请求头注入 Authorization: Bearer <token>
- 后端拦截器校验 Token，通过后将用户上下文放入当前线程
- 控制器调用服务层执行业务逻辑，返回统一响应结构
- 新增的用户信息修改和密码修改接口通过拦截器获取当前用户ID并执行相应业务

```mermaid
sequenceDiagram
participant F as "前端"
participant R as "Axios 请求封装"
participant I as "认证拦截器"
participant C as "AuthController"
participant S as "AuthServiceImpl"
participant M as "UserMapper"
participant J as "JwtUtil"
F->>R : "发起登录/业务请求"
R->>I : "携带 Authorization 头"
I->>J : "校验 Token"
J-->>I : "通过/失败"
I-->>C : "放行并注入用户上下文"
C->>S : "调用业务方法"
S->>M : "数据库读写"
M-->>S : "返回结果"
S-->>C : "返回数据"
C-->>F : "统一响应 Result"
```

图表来源
- [AuthInterceptor.java:30-58](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java#L30-L58)
- [AuthController.java:25-71](file://backend/src/main/java/com/newworld/controller/AuthController.java#L25-L71)
- [AuthServiceImpl.java:40-95](file://backend/src/main/java/com/newworld/service/impl/AuthServiceImpl.java#L40-L95)
- [JwtUtil.java:61-69](file://backend/src/main/java/com/newworld/common/JwtUtil.java#L61-L69)
- [request.js:9-19](file://frontend/src/utils/request.js#L9-L19)

## 详细组件分析

### 接口定义与使用说明

#### 登录接口
- 方法与路径：POST /api/auth/login
- 请求体：LoginRequest（用户名、密码）
- 成功响应：Result，data 包含 token 字段
- 错误响应：Result，code 非 200，msg 描述错误
- 状态码：200 表示成功；400/401/500 由全局异常处理器映射
- 示例
  - 成功：返回包含 token 的 Result
  - 密码错误/用户名不存在：返回错误 Result
- 安全提示：密码以明文传输前会进行 SHA-256 加密存储

#### 注册接口
- 方法与路径：POST /api/auth/register
- 请求体：LoginRequest（用户名、密码）
- 成功响应：Result，msg 表示"注册成功"
- 错误响应：Result，如用户名已存在

#### 获取当前用户信息
- 方法与路径：GET /api/auth/user-info
- 需要登录态（拦截器放行）
- 成功响应：Result，data 为 User 对象（不包含密码）

#### 修改用户信息
- 方法与路径：PUT /api/auth/user-info
- 请求体：UpdateUserRequest（昵称）
- 需要登录态（拦截器放行）
- 成功响应：Result，msg 表示"修改成功"
- 安全验证：仅更新非空昵称字段，用户存在性验证

#### 修改密码
- 方法与路径：PUT /api/auth/password
- 请求体：ChangePasswordRequest（旧密码、新密码）
- 需要登录态（拦截器放行）
- 成功响应：Result，msg 表示"密码修改成功"
- 安全验证：旧密码验证、新密码长度不少于6位

#### 退出接口
- 方法与路径：POST /api/auth/logout
- 需要登录态
- 成功响应：Result，msg 表示"退出成功"

**更新** 新增用户信息修改和密码修改接口，完善了用户的个人信息维护能力

**章节来源**
- [AuthController.java:25-71](file://backend/src/main/java/com/newworld/controller/AuthController.java#L25-L71)
- [LoginRequest.java:1-37](file://backend/src/main/java/com/newworld/dto/LoginRequest.java#L1-L37)
- [UpdateUserRequest.java:1-19](file://backend/src/main/java/com/newworld/dto/UpdateUserRequest.java#L1-L19)
- [ChangePasswordRequest.java:1-36](file://backend/src/main/java/com/newworld/dto/ChangePasswordRequest.java#L1-L36)
- [Result.java:22-50](file://backend/src/main/java/com/newworld/common/Result.java#L22-L50)
- [GlobalExceptionHandler.java:17-21](file://backend/src/main/java/com/newworld/common/exception/GlobalExceptionHandler.java#L17-L21)
- [User.java:1-95](file://backend/src/main/java/com/newworld/entity/User.java#L1-L95)

### JWT Token 生成、验证与刷新机制

#### 生成
- 使用 JwtUtil 生成 Token，载荷包含用户 ID 与用户名，签名算法为 HS512，有效期由配置决定

#### 验证
- 拦截器从请求头 Authorization 中提取 Token，去除 Bearer 前缀后调用 JwtUtil.validateToken 校验

#### 刷新
- 当前实现未提供专用刷新接口；可按需扩展"刷新 Token"接口，或在前端检测到 401 时引导重新登录

```mermaid
flowchart TD
Start(["进入拦截器"]) --> GetHeader["读取 Authorization 头"]
GetHeader --> HasToken{"是否存在 Token?"}
HasToken --> |否| Err401["抛出 401 未登录"]
HasToken --> |是| Trim["去除 Bearer 前缀"]
Trim --> Validate["JwtUtil.validateToken()"]
Validate --> Valid{"是否有效?"}
Valid --> |否| Err401_2["抛出 401 Token 过期/无效"]
Valid --> |是| Parse["解析用户ID/用户名"]
Parse --> SetCtx["设置当前用户上下文(ThreadLocal)"]
SetCtx --> End(["放行请求"])
```

图表来源
- [AuthInterceptor.java:30-58](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java#L30-L58)
- [JwtUtil.java:61-69](file://backend/src/main/java/com/newworld/common/JwtUtil.java#L61-L69)

**章节来源**
- [JwtUtil.java:29-40](file://backend/src/main/java/com/newworld/common/JwtUtil.java#L29-L40)
- [application.yml:65-68](file://backend/src/main/resources/application.yml#L65-L68)
- [AuthInterceptor.java:37-49](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java#L37-L49)

### 认证拦截器与请求头传递

#### 拦截范围
- 对 /api/** 生效，排除 /api/auth/login、/api/auth/register、/api/system/** 等

#### 请求头传递
- 前端在请求拦截中自动添加 Authorization: Bearer <token>
- 后端从 Authorization 头解析并校验

#### 上下文注入
- 校验通过后将用户 ID 与用户名放入 ThreadLocal，供控制器读取

```mermaid
sequenceDiagram
participant FE as "前端"
participant AX as "Axios 拦截器"
participant IN as "AuthInterceptor"
participant CT as "AuthController"
FE->>AX : "发送请求"
AX->>AX : "若存在 token 则设置 Authorization"
AX->>IN : "请求到达后端"
IN->>IN : "校验 Token"
IN->>CT : "放行并注入用户上下文"
CT-->>FE : "返回统一响应"
```

图表来源
- [WebMvcConfig.java:19-33](file://backend/src/main/java/com/newworld/config/WebMvcConfig.java#L19-L33)
- [request.js:9-19](file://frontend/src/utils/request.js#L9-L19)
- [AuthInterceptor.java:30-58](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java#L30-L58)

**章节来源**
- [WebMvcConfig.java:19-33](file://backend/src/main/java/com/newworld/config/WebMvcConfig.java#L19-L33)
- [AuthInterceptor.java:30-58](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java#L30-L58)
- [request.js:9-19](file://frontend/src/utils/request.js#L9-L19)

### 业务流程与数据模型

#### 登录流程
- 查询用户是否存在
- 校验密码（SHA-256）
- 生成并返回 Token

#### 注册流程
- 校验用户名唯一性
- 创建用户并保存（密码 SHA-256）

#### 用户信息查询
- 通过拦截器注入的用户 ID 查询用户并返回（密码置空）

#### 用户信息修改流程
- 通过拦截器获取当前用户ID
- 校验用户存在性
- 更新非空昵称字段
- 返回修改成功

#### 密码修改流程
- 通过拦截器获取当前用户ID
- 校验用户存在性
- 验证旧密码（SHA-256）
- 更新为新密码（SHA-256）
- 返回修改成功

```mermaid
flowchart TD
LStart["POST /api/auth/login"] --> FindUser["根据用户名查询用户"]
FindUser --> Exists{"用户存在?"}
Exists --> |否| ErrUE["抛出用户名或密码错误"]
Exists --> |是| HashPwd["对输入密码做 SHA-256"]
HashPwd --> Match{"密码匹配?"}
Match --> |否| ErrPE["抛出用户名或密码错误"]
Match --> |是| GenToken["JwtUtil 生成 Token"]
GenToken --> LEnd["返回 Result{token}"]
RStart["POST /api/auth/register"] --> CheckDup["检查用户名唯一"]
CheckDup --> Dup{"已存在?"}
Dup --> |是| ErrDup["抛出用户名已存在"]
Dup --> |否| SaveUser["创建用户并保存"]
SaveUser --> REnd["返回注册成功"]
UIStart["PUT /api/auth/user-info"] --> GetUID["拦截器获取用户ID"]
GetUID --> LoadUser["查询用户信息"]
LoadUser --> UserExists{"用户存在?"}
UserExists --> |否| ErrUser["抛出用户不存在"]
UserExists --> |是| UpdateNick["更新昵称字段"]
UpdateNick --> SaveUser2["保存用户信息"]
SaveUser2 --> UIEnd["返回修改成功"]
CPStart["PUT /api/auth/password"] --> GetUID2["拦截器获取用户ID"]
GetUID2 --> LoadUser2["查询用户信息"]
LoadUser2 --> UserExists2{"用户存在?"}
UserExists2 --> |否| ErrUser2["抛出用户不存在"]
UserExists2 --> |是| HashOld["验证旧密码 SHA-256"]
HashOld --> OldMatch{"旧密码正确?"}
OldMatch --> |否| ErrOld["抛出旧密码不正确"]
OldMatch --> |是| UpdatePass["更新为新密码 SHA-256"]
UpdatePass --> SaveUser3["保存用户信息"]
SaveUser3 --> CPEnd["返回密码修改成功"]
```

图表来源
- [AuthServiceImpl.java:40-95](file://backend/src/main/java/com/newworld/service/impl/AuthServiceImpl.java#L40-L95)
- [UserMapper.java:1-10](file://backend/src/main/java/com/newworld/mapper/UserMapper.java#L1-L10)
- [JwtUtil.java:29-40](file://backend/src/main/java/com/newworld/common/JwtUtil.java#L29-L40)

**章节来源**
- [AuthServiceImpl.java:24-95](file://backend/src/main/java/com/newworld/service/impl/AuthServiceImpl.java#L24-L95)
- [UserMapper.java:1-10](file://backend/src/main/java/com/newworld/mapper/UserMapper.java#L1-L10)
- [User.java:1-95](file://backend/src/main/java/com/newworld/entity/User.java#L1-L95)

### 前端集成要点
- 登录成功后将 token 写入本地存储，并在后续请求中自动附加 Authorization 头
- 401 时清除本地 token 并跳转至登录页
- 获取用户信息接口用于初始化用户资料
- 用户信息修改和密码修改接口支持在线个人资料维护

**更新** 前端需要支持新的用户信息修改和密码修改接口调用

**章节来源**
- [auth.js:1-14](file://frontend/src/api/auth.js#L1-L14)
- [auth.store.js:1-41](file://frontend/src/stores/auth.store.js#L1-L41)
- [request.js:9-19](file://frontend/src/utils/request.js#L9-L19)

## 依赖分析
- 控制器依赖服务层与统一响应
- 服务层依赖 Mapper、JwtUtil 与异常处理
- 拦截器依赖 JwtUtil 与全局异常
- MVC 配置注册拦截器并排除特定路径
- 前端通过 Axios 统一注入 Authorization 头
- 新增 DTO 层提供参数验证支持

```mermaid
graph LR
AC["AuthController"] --> ASI["AuthServiceImpl"]
ASI --> UM["UserMapper"]
ASI --> JWT["JwtUtil"]
INT["AuthInterceptor"] --> JWT
CFG["WebMvcConfig"] --> INT
EX["GlobalExceptionHandler"] --> RES["Result"]
FE["前端"] --> AX["Axios 拦截器"]
AX --> AC
AC --> D1["UpdateUserRequest"]
AC --> D2["ChangePasswordRequest"]
```

图表来源
- [AuthController.java:1-73](file://backend/src/main/java/com/newworld/controller/AuthController.java#L1-L73)
- [AuthServiceImpl.java:1-97](file://backend/src/main/java/com/newworld/service/impl/AuthServiceImpl.java#L1-L97)
- [AuthInterceptor.java:1-78](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java#L1-L78)
- [WebMvcConfig.java:1-53](file://backend/src/main/java/com/newworld/config/WebMvcConfig.java#L1-L53)
- [Result.java:1-90](file://backend/src/main/java/com/newworld/common/Result.java#L1-L90)
- [GlobalExceptionHandler.java:1-35](file://backend/src/main/java/com/newworld/common/exception/GlobalExceptionHandler.java#L1-L35)
- [request.js:9-19](file://frontend/src/utils/request.js#L9-L19)
- [UpdateUserRequest.java:1-19](file://backend/src/main/java/com/newworld/dto/UpdateUserRequest.java#L1-L19)
- [ChangePasswordRequest.java:1-36](file://backend/src/main/java/com/newworld/dto/ChangePasswordRequest.java#L1-L36)

**章节来源**
- [AuthController.java:1-73](file://backend/src/main/java/com/newworld/controller/AuthController.java#L1-L73)
- [AuthServiceImpl.java:1-97](file://backend/src/main/java/com/newworld/service/impl/AuthServiceImpl.java#L1-L97)
- [AuthInterceptor.java:1-78](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java#L1-L78)
- [WebMvcConfig.java:1-53](file://backend/src/main/java/com/newworld/config/WebMvcConfig.java#L1-L53)
- [GlobalExceptionHandler.java:1-35](file://backend/src/main/java/com/newworld/common/exception/GlobalExceptionHandler.java#L1-L35)
- [request.js:9-19](file://frontend/src/utils/request.js#L9-L19)

## 性能考量
- Token 有效期默认 24 小时，可根据业务调整
- 拦截器每次请求都会校验 Token，建议保持合理的日志级别避免过多开销
- 前端统一注入 Authorization 头，减少重复代码与错误
- 用户信息修改和密码修改操作均为轻量级数据库更新，性能开销较小

## 故障排查指南
- 401 未登录/Token 无效
  - 检查前端是否正确设置 Authorization 头
  - 检查后端拦截器是否正确去除 Bearer 前缀
  - 检查 JWT secret 与 expiration 配置是否一致

- 用户名或密码错误
  - 确认用户名存在且密码 SHA-256 匹配

- 用户名已存在
  - 注册时检查用户名唯一性

- 用户不存在
  - 用户信息修改和密码修改前会进行用户存在性验证

- 旧密码不正确
  - 密码修改接口会验证旧密码的正确性

- 新密码格式错误
  - 新密码长度不少于6位

- 服务器内部错误
  - 查看全局异常处理器返回的错误信息

**更新** 新增用户信息修改和密码修改相关的错误处理指引

**章节来源**
- [AuthInterceptor.java:37-49](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java#L37-L49)
- [GlobalExceptionHandler.java:17-33](file://backend/src/main/java/com/newworld/common/exception/GlobalExceptionHandler.java#L17-L33)
- [application.yml:65-68](file://backend/src/main/resources/application.yml#L65-L68)
- [AuthServiceImpl.java:60-95](file://backend/src/main/java/com/newworld/service/impl/AuthServiceImpl.java#L60-L95)

## 结论
本认证体系基于 JWT 实现，具备清晰的拦截链路与统一响应结构。登录/注册流程简单可靠，拦截器确保受保护接口的安全访问。新增的用户信息修改和密码修改接口完善了用户个人信息维护能力，配合参数验证确保数据完整性。建议后续补充 Token 刷新与登出清理策略，进一步提升用户体验与安全性。

## 附录

### 请求与响应示例（描述性）

#### 登录成功
- 请求：POST /api/auth/login，Body 包含用户名与密码
- 响应：Result，code=200，data.token 存在

#### 密码错误
- 响应：Result，code 非 200，msg 为"用户名或密码错误"

#### 未登录
- 响应：Result，code=401，msg 为"未登录，请先登录"

#### Token 过期/无效
- 响应：Result，code=401，msg 为"Token 已过期或无效，请重新登录"

#### 用户信息修改成功
- 请求：PUT /api/auth/user-info，Body 包含 nickname
- 响应：Result，code=200，msg 为"修改成功"

#### 密码修改成功
- 请求：PUT /api/auth/password，Body 包含 oldPassword 和 newPassword
- 响应：Result，code=200，msg 为"密码修改成功"

#### 参数验证错误
- 用户信息修改：nickname 为空时返回验证错误
- 密码修改：oldPassword 或 newPassword 为空时返回验证错误
- 密码修改：newPassword 长度少于6位时返回验证错误

**更新** 新增用户信息修改和密码修改的请求响应示例

**章节来源**
- [AuthController.java:25-71](file://backend/src/main/java/com/newworld/controller/AuthController.java#L25-L71)
- [AuthServiceImpl.java:40-95](file://backend/src/main/java/com/newworld/service/impl/AuthServiceImpl.java#L40-L95)
- [AuthInterceptor.java:37-49](file://backend/src/main/java/com/newworld/config/AuthInterceptor.java#L37-L49)
- [Result.java:22-50](file://backend/src/main/java/com/newworld/common/Result.java#L22-L50)
- [UpdateUserRequest.java:1-19](file://backend/src/main/java/com/newworld/dto/UpdateUserRequest.java#L1-L19)
- [ChangePasswordRequest.java:1-36](file://backend/src/main/java/com/newworld/dto/ChangePasswordRequest.java#L1-L36)